/*
 * @Author: your name
 * @Date: 2023-11-04 22:26:05
 * @LastEditTime: 2023-11-04 23:20:12
 * @LastEditors: LAPTOP-HVHI0UBN
 * @Description: In User Settings Edit
 * @FilePath: \vuecli-demo\src\utils\xlsxStyle.utils.js
 */
/*
@author Ctrl
@version 20190917
...
XSU = {
  ...
  init: function (workBook, sheetName, cell) {
    console.log(workBook.Sheets[sheetName][cell].s);
  ...
  mergeCells: function (workBook, sheetName, startCell, endCell) {
    ...
    if (!workBook.Sheets[sheetName]["!merges"]) {
      ...
      workBook.Sheets[sheetName]["!merges"] =
        workBook.Sheets[sheetName]["!merges"].concat(merges);
    }
  ...
  setBorderDiagonalDefault: function (workBook, sheetName, cell) {
    this.init1(workBook, sheetName, cell, "border");
    workBook.Sheets[sheetName][cell].s.border.diagonal =
      ...
      for (var j = i + 1; j < cells.length; j++) {
        ...
        if (
          workBook.Sheets[sheetName][cells[i]].v ==
            workBook.Sheets[sheetName][cells[j]].v &&
          workBook.Sheets[sheetName][cells[i]].v != ""
        ) {
